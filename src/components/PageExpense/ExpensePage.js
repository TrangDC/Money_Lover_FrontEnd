...
const ExpensePage = ({wallet_id}) => {
    ...
    function getlist(transactions) {
        ...
        transactions.forEach(transaction => {
            // nếu transaction thuộc type expense
            if (transaction.category.type === "EXPENSE") {
                const index = exCategory.indexOf(transaction.category.name)
                if (index === -1) {
                    exCategory.push(transaction.category.name);
                    exAmount.push(transaction.amount);
                }
                exAmount[index] += transaction.amount
            }
        })
        setExCategory(exCategory);
        setExAmount(exAmount)
    }
    const data = {
        labels: exCategory,
        datasets: [{
